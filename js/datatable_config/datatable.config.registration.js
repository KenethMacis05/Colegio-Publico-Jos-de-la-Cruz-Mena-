...
const dataTableOptions = {
    //scrollX: "2000px",
    lengthMenu: [3, 10, 15, 20, 100, 200, 500],
    columnDefs: [
...
const listUsers = async () => {
    try {
        ...
        retButtons.forEach(button => {
            button.addEventListener('click', () => {
                const userIndex2 = button.dataset.userIndex; // Obtener índice del usuario del atributo data
                alertRetiro(userIndex2); // Llamar a la función alertModificar con el índice de usuario
            });
        });
    } catch (ex) {
        alert(ex);
